...
static const uint8_t digit_segment_mapping[128] = {
	...
	0, 			0b01011111,	0b01111001, 0b00110001, 0b01110011, 0b01111100,	0b00011101, 0,			// char 40 to 47 @ABCDEFG
	...
	0, 			0b01011111,	0b01111001, 0b00110001, 0b01110011, 0b01111100,	0b00011101, 0,			// char 60 to 67 `abcdefg
...
public:
	...
	{
		D_PRINT(6, "%s(%s, %d, %06X);", __ASSERT_FUNC, string, length, colour);
		length %= 6;
	...
	{
		D_PRINT(6, "%s(%d, %06X);", __ASSERT_FUNC, seconds, colour);
		show_time(seconds, 0, colour, colour);
	}

	void show_time(uint16_t seconds, uint8_t digit_select, CRGB colour, CRGB highlight) {
		D_PRINT(6, "%s(%d, %d, %06X, %06X);", __ASSERT_FUNC, seconds, digit_select, colour, highlight);
